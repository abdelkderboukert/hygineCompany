"use client";
import React from "react";
import * as motion from "motion/react-client";
import Link from "next/link";
import { useRouter } from "next/navigation";

const Page = () => {
  const router = useRouter();

  const handleLogout = async () => {
    const res = await fetch("/api/logout", {
      method: "POST",
    });

    const data = await res.json();

    if (data.success) {
      router.push("/login");
    }
  };
  return (
    <section className="lg:h-screen h-full w-screen mb-10" id="service">
      <br />
      <br />
      <motion.h1
        initial={{ opacity: 0, y: 30 }}
        whileInView={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.75 }}
        className="flex mt-14 justify-center mx-auto lg:text-7xl md:text-4xl text-[#0e012d] font-semibold"
      >
        <Link href="/serves/">Admin&nbsp;Page </Link>
      </motion.h1>
      {/* <motion.div
        initial={{ opacity: 0, y: 30 }}
        whileInView={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.75 }}
        className="flex w-full h-max mt-5 justify-center items-center mx-auto select-none text-zinc-400 text-center text-xl"
      >
        Au-dela de la Fourmniture de prduits et d&apos;equipements
        d&apos;hygiene pour l&apos;hygiene pour <br /> l&apos;industrie
        agroalimentaire, Hygindust & Hyprotech vous accompagent a travers
      </motion.div> */}
      <div
        className=" grid grid-flow-row px-28 md:grid-cols-2 lg:grid-cols-4 p-10 gap-16 lg:gap-12 w-screen h-max"
        content=""
      >
        <motion.div
          whileHover={{ scale: 1.1 }}
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          //   transition={{ duration: 0.75 }}
          className="h-96 w-full flex flex-col"
        >
          <Link
            href="admin/Equipements/add"
            className="flex w-full h-3/6 rounded-t-xl justify-center items-center"
          >
            <div className="w-36 h-36 bg-purple-200 rounded-full flex justify-center items-center ">
              <div className="h-20 w-20 text-purple-900 flex justify-center items-center text-7xl">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  height="60px"
                  viewBox="0 -960 960 960"
                  width="60px"
                  fill="#581c87"
                >
                  <path d="m352-522 86-87-56-57-44 44-56-56 43-44-45-45-87 87 159 158Zm328 329 87-87-45-45-44 43-56-56 43-44-57-56-86 86 158 159Zm24-567 57 57-57-57ZM290-120H120v-170l175-175L80-680l200-200 216 216 151-152q12-12 27-18t31-6q16 0 31 6t27 18l53 54q12 12 18 27t6 31q0 16-6 30.5T816-647L665-495l215 215L680-80 465-295 290-120Zm-90-80h56l392-391-57-57-391 392v56Zm420-419-29-29 57 57-28-28Z" />
                </svg>
              </div>
            </div>
          </Link>
          {/* <h1 className="text-2xl font-semibold mx-auto text-black"> */}
          <Link
            className="text-2xl font-semibold mx-auto text-black"
            href="admin/Equipements/add"
          >
            Equipements ADD
          </Link>
          {/* </h1> */}
          <br />
          <span className="text-center text-zinc-400">
            {" "}
            add prudect or type/subtype
          </span>
        </motion.div>
        <motion.div
          whileHover={{ scale: 1.1 }}
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          //   transition={{ duration: 0.75 }}
          className="h-96 w-full rounded-xl flex flex-col border-black"
        >
          <Link
            href="admin/Equipements/edit"
            className="flex w-full h-3/6 rounded-t-xl justify-center items-center"
          >
            <div className="w-36 h-36 bg-blue-200 rounded-full flex justify-center items-center ">
              <div className="h-20 w-20 text-blue-900 flex justify-center items-center">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  height="60px"
                  viewBox="0 -960 960 960"
                  width="60px"
                  fill="#1e3a8a"
                >
                  <path d="M42-120v-112q0-33 17-62t47-44q51-26 115-44t141-18q77 0 141 18t115 44q30 15 47 44t17 62v112H42Zm80-80h480v-32q0-11-5.5-20T582-266q-36-18-92.5-36T362-320q-71 0-127.5 18T142-266q-9 5-14.5 14t-5.5 20v32Zm240-240q-66 0-113-47t-47-113h-10q-9 0-14.5-5.5T172-620q0-9 5.5-14.5T192-640h10q0-45 22-81t58-57v38q0 9 5.5 14.5T302-720q9 0 14.5-5.5T322-740v-54q9-3 19-4.5t21-1.5q11 0 21 1.5t19 4.5v54q0 9 5.5 14.5T422-720q9 0 14.5-5.5T442-740v-38q36 21 58 57t22 81h10q9 0 14.5 5.5T552-620q0 9-5.5 14.5T532-600h-10q0 66-47 113t-113 47Zm0-80q33 0 56.5-23.5T442-600H282q0 33 23.5 56.5T362-520Zm300 160-6-30q-6-2-11.5-4.5T634-402l-28 10-20-36 22-20v-24l-22-20 20-36 28 10q4-4 10-7t12-5l6-30h40l6 30q6 2 12 5t10 7l28-10 20 36-22 20v24l22 20-20 36-28-10q-5 5-10.5 7.5T708-390l-6 30h-40Zm20-70q12 0 21-9t9-21q0-12-9-21t-21-9q-12 0-21 9t-9 21q0 12 9 21t21 9Zm72-130-8-42q-9-3-16.5-7.5T716-620l-42 14-28-48 34-30q-2-5-2-8v-16q0-3 2-8l-34-30 28-48 42 14q6-6 13.5-10.5T746-798l8-42h56l8 42q9 3 16.5 7.5T848-780l42-14 28 48-34 30q2 5 2 8v16q0 3-2 8l34 30-28 48-42-14q-6 6-13.5 10.5T818-602l-8 42h-56Zm28-90q21 0 35.5-14.5T832-700q0-21-14.5-35.5T782-750q-21 0-35.5 14.5T732-700q0 21 14.5 35.5T782-650ZM362-200Z" />
                </svg>
              </div>
            </div>
          </Link>
          <Link
            className="text-2xl font-semibold mx-auto text-black"
            href="admin/Equipements/edit"
          >
            Equipements Edit
          </Link>
          <br />
          <span className="text-center text-zinc-400">
            {" "}
            edit prudect and delet theme
          </span>
        </motion.div>
        <motion.div
          whileHover={{ scale: 1.1 }}
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          //   transition={{ duration: 0.75 }}
          className="h-96 w-full flex flex-col"
        >
          <Link
            href="admin/product/add"
            className="flex w-full h-3/6 rounded-t-xl justify-center items-center"
          >
            <div className="w-36 h-36 bg-purple-200 rounded-full flex justify-center items-center ">
              <div className="h-20 w-20 text-purple-900 flex justify-center items-center text-7xl">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  height="60px"
                  viewBox="0 -960 960 960"
                  width="60px"
                  fill="#581c87"
                >
                  <path d="m352-522 86-87-56-57-44 44-56-56 43-44-45-45-87 87 159 158Zm328 329 87-87-45-45-44 43-56-56 43-44-57-56-86 86 158 159Zm24-567 57 57-57-57ZM290-120H120v-170l175-175L80-680l200-200 216 216 151-152q12-12 27-18t31-6q16 0 31 6t27 18l53 54q12 12 18 27t6 31q0 16-6 30.5T816-647L665-495l215 215L680-80 465-295 290-120Zm-90-80h56l392-391-57-57-391 392v56Zm420-419-29-29 57 57-28-28Z" />
                </svg>
              </div>
            </div>
          </Link>
          {/* <h1 className="text-2xl font-semibold mx-auto text-black"> */}
          <Link
            className="text-2xl font-semibold mx-auto text-black"
            href="admin/product/add"
          >
            Product ADD
          </Link>
          {/* </h1> */}
          <br />
          <span className="text-center text-zinc-400">
            {" "}
            add prudect or type/subtype
          </span>
        </motion.div>
        <motion.div
          whileHover={{ scale: 1.1 }}
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          //   transition={{ duration: 0.75 }}
          className="h-96 w-full rounded-xl flex flex-col border-black"
        >
          <Link
            href="admin/product/edit"
            className="flex w-full h-3/6 rounded-t-xl justify-center items-center"
          >
            <div className="w-36 h-36 bg-blue-200 rounded-full flex justify-center items-center ">
              <div className="h-20 w-20 text-blue-900 flex justify-center items-center">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  height="60px"
                  viewBox="0 -960 960 960"
                  width="60px"
                  fill="#1e3a8a"
                >
                  <path d="M42-120v-112q0-33 17-62t47-44q51-26 115-44t141-18q77 0 141 18t115 44q30 15 47 44t17 62v112H42Zm80-80h480v-32q0-11-5.5-20T582-266q-36-18-92.5-36T362-320q-71 0-127.5 18T142-266q-9 5-14.5 14t-5.5 20v32Zm240-240q-66 0-113-47t-47-113h-10q-9 0-14.5-5.5T172-620q0-9 5.5-14.5T192-640h10q0-45 22-81t58-57v38q0 9 5.5 14.5T302-720q9 0 14.5-5.5T322-740v-54q9-3 19-4.5t21-1.5q11 0 21 1.5t19 4.5v54q0 9 5.5 14.5T422-720q9 0 14.5-5.5T442-740v-38q36 21 58 57t22 81h10q9 0 14.5 5.5T552-620q0 9-5.5 14.5T532-600h-10q0 66-47 113t-113 47Zm0-80q33 0 56.5-23.5T442-600H282q0 33 23.5 56.5T362-520Zm300 160-6-30q-6-2-11.5-4.5T634-402l-28 10-20-36 22-20v-24l-22-20 20-36 28 10q4-4 10-7t12-5l6-30h40l6 30q6 2 12 5t10 7l28-10 20 36-22 20v24l22 20-20 36-28-10q-5 5-10.5 7.5T708-390l-6 30h-40Zm20-70q12 0 21-9t9-21q0-12-9-21t-21-9q-12 0-21 9t-9 21q0 12 9 21t21 9Zm72-130-8-42q-9-3-16.5-7.5T716-620l-42 14-28-48 34-30q-2-5-2-8v-16q0-3 2-8l-34-30 28-48 42 14q6-6 13.5-10.5T746-798l8-42h56l8 42q9 3 16.5 7.5T848-780l42-14 28 48-34 30q2 5 2 8v16q0 3-2 8l34 30-28 48-42-14q-6 6-13.5 10.5T818-602l-8 42h-56Zm28-90q21 0 35.5-14.5T832-700q0-21-14.5-35.5T782-750q-21 0-35.5 14.5T732-700q0 21 14.5 35.5T782-650ZM362-200Z" />
                </svg>
              </div>
            </div>
          </Link>
          <Link
            className="text-2xl font-semibold mx-auto text-black"
            href="admin/product/edit"
          >
            Product Edit
          </Link>
          <br />
          <span className="text-center text-zinc-400">
            {" "}
            edit prudect and delet theme
          </span>
        </motion.div>
      </div>
      <button
        className="w-40 h-14 bg-red-600 rounded-lg mt-5 text-white font-bold"
        onClick={handleLogout}
      >
        Logout
      </button>
    </section>
  );
};

export default Page;
